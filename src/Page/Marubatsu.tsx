...
const checkResult = (selectedItems: SelectItemsType[], itemType: string) => {
  ...
  const selectedPosition = selectedItems.map((v, i) =>
    v.item === itemValue ? positionNumbers[i] : 0
  )

  // rows
  if (selectedPosition[0] + selectedPosition[1] + selectedPosition[2] == 7)
    return true
  if (selectedPosition[3] + selectedPosition[4] + selectedPosition[5] == 56)
    return true
  if (selectedPosition[6] + selectedPosition[7] + selectedPosition[8] == 448)
    return true
  // columns
  if (selectedPosition[0] + selectedPosition[3] + selectedPosition[6] == 73)
    return true
  if (selectedPosition[1] + selectedPosition[4] + selectedPosition[7] == 146)
    return true
  if (selectedPosition[2] + selectedPosition[5] + selectedPosition[8] == 292)
    return true
  // diagonal
  if (selectedPosition[0] + selectedPosition[4] + selectedPosition[8] == 273)
    return true
  if (selectedPosition[6] + selectedPosition[4] + selectedPosition[2] == 84)
    return true
  return false
}
