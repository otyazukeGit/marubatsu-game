  ...
  switch (action.type) {
    ...
    case 'setResult': {
      ...
      const newGameRecord = state.gameRecord.map((record) =>
        record.winner === action.winner
          ? Object.assign({}, record, { ...record, count: record.count + 1 })
          : record
      )
      return Object.assign({}, state, {
        ...state,
        resultMessaage: action.msg,
        finish: true,
        gameRecord: JSON.parse(JSON.stringify(newGameRecord)),
      })
    }
    case 'signIn':
      console.log('signIn')
    ...
    case 'inputValidateSignUp':
      newValidateItem = Object.assign(
        {},
        JSON.parse(JSON.stringify(defaultInputValidate))
      ) // clear validation states.
      newValidateItem.signUp[action.item].error = true
      newValidateItem.signUp[action.item].message = action.msg
      return Object.assign({}, state, {
        ...state,
        inputValidate: newValidateItem,
      })
