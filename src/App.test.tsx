...
const Test_HeaderArea = () => {
  /* Header Area */
  expect(
    screen.getByRole('heading', { name: 'MaruBatsu Game' })
  ).toBeInTheDocument()
...
test('Home page : Initial Display', async () => {
  ...
  expect(
    screen.getByText(/Have you ever heard of Marubatsu Game, or TIC TAC TOE\?/)
  ).toBeInTheDocument()
  expect(
    screen.getByText(
      /Marubatsu Game is a paper-and-pencil game for two players, X and O, who take turns marking the spaces in a 3×3 grid./
    )
  ).toBeInTheDocument()
  expect(
    screen.getByText(
  ...
  expect(
    screen.getByText(/Please join us and enjoy the Marubatsu Game!/)
  ).toBeInTheDocument()
...
test('Sign Up page : Sign Up', async () => {
  ...
  expect(
    screen.getByText('Password (at least 6 characters)')
  ).toBeInTheDocument() // "*" is pseudo-element(before)
  expect(
    screen.getByText('Confirm Password (at least 6 characters)')
  ).toBeInTheDocument() // "*" is pseudo-element(before)
  expect(
  ...
  fireEvent.input(emailAddressInput, {
    target: { value: 'emu-test@gmail.com' },
  })
